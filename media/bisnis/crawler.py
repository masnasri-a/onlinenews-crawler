...
def crawler():
    for link in list_link:
        try:
            ...
            author = str(desc.find('a').text).replace(
                '  ', '').replace('\n', '').replace('\r', '')

            created_at = desc.find('span')
            dates = str(created_at.text).replace('  ', '').replace('\n', '').replace('|', '').replace(
                '\r', '').replace('\xa0\xa0 ', '').replace('WIB', '').replace(':', ' ').split(' ')
            ...
            for detail in list_article:
                articles.append(str(detail.text).replace('Bisnis.com, ', '').replace('\r\n', '').replace('  ', '').replace(
                    '. Simak berita lainnya seputar topikartikel ini, di sini :  Bergabung dan dapatkan analisis informasi ekonomi dan bisnis melalui email Anda.', ''))
